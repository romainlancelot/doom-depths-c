...
char *save_player(Player *player)
{
    ...
    char *base = "INSERT INTO players (current_health, max_health, current_mana, max_mana, gold, experience, level, defense, attack_power, attack_left, potion_counter, stage) VALUES";
    sprintf(sql, "%s (%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d);", base, player->current_health, player->max_health, player->current_mana, player->max_mana, player->gold, player->experience, player->level, player->defense, player->attack_power, player->attack_left, player->potion_counter, player->stage);
    return sql;
}
...
{
    char *sql = malloc(1000 * sizeof(char));
    char *base = "UPDATE players SET current_health = %d, max_health = %d, current_mana = %d, max_mana = %d, gold = %d, experience = %d, level = %d, defense = %d, attack_power = %d, attack_left = %d, potion_counter = %d, stage = %d WHERE id = %d;";
    sprintf(sql, base, player->current_health, player->max_health, player->current_mana, player->max_mana, player->gold, player->experience, player->level, player->defense, player->attack_power, player->attack_left, player->potion_counter, player->stage, player->id);
    return sql;
}
